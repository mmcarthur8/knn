...
## new household new.df <- data.frame(Income = 60, Lot_Size = 20) 
...
library(FNN) 
nn <- knn(train = train.norm.df[, 1:2], test = new.norm.df, cl = train.norm.df[, 3], k = 3) row.names(train.df)[attr(nn, "nn.index")] 
